...
class StepperControllerTest(unittest.TestCase):
    ...
    def test_clockwise(self):
        ...
        pid = PID(1, 0.1, 0.1, 0.1, limits = (0, 2 * np.pi))
        controller = StepperController(stepper, pid)
        controller.move(1)
        self.assertEqual(controller.get_steps(), 0)
        self.assertAlmostEqual(controller.get_angle(), 0)
        controller.move(0.5)
        self.assertEqual(controller.get_steps(), 50)
        self.assertAlmostEqual(controller.get_angle(), np.pi)
        controller.move(0.6)
        self.assertEqual(controller.get_steps(), 10)
        self.assertAlmostEqual(controller.get_angle(), np.pi * 0.2)
